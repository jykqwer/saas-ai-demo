...
async def get_session(request: Request, session_id: UUID) -> SessionMessagesResponse:
    repo = request.app.state.chat_repository
...
async def delete_session(request: Request, session_id: UUID) -> DeleteResponse:
    repo = request.app.state.chat_repository
    deleted = await repo.delete_session(session_id=session_id)
